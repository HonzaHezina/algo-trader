...
function buildTradeCardMessage(tradeCard) {
  ...
  if (mindsdb_prediction && mindsdb_prediction.direction) {
    const movePct = mindsdb_prediction.predicted_move_pct != null
      ? `, ${mindsdb_prediction.predicted_move_pct > 0 ? '+' : ''}${mindsdb_prediction.predicted_move_pct}% predikce`
      : '';
    lines.push(`🤖 MindsDB: ${mindsdb_prediction.direction}${movePct}`);
  }
